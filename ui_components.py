...
class ExcelConfigDialog(tk.Toplevel):
    ...
    def _setup_ui(self):
        ...
        self.tab_cb = ttk.Combobox(
            container, textvariable=self.tab_var, values=self.xl.sheet_names, state="readonly"
        )
    ...
    def _on_tab_select(self, _):
        ...
        self.cb_id.config(values=cols)  # type: ignore
        self.cb_hex.config(values=cols)  # type: ignore
        self.cb_meaning.config(values=cols)  # type: ignore
