...
class CANProAnalyzer:
    ...
    def _init_ui(self):
        ...
        scr_l.pack(side=tk.RIGHT, fill=tk.Y)
        ...
        self.ins_tree = ttk.Treeview(
            r_frame, columns=("Signal", "Value", "Translation", "Unit"), show="headings"
        )
        ...
        self.log_text = tk.Text(
            log_frame, height=8, bg="#1e272e", fg="#00d8d6", font=("Consolas", 9), state="disabled"
        )
    ...
    def _setup_table_view(self, parent):
        ...
        self.data_tree = ttk.Treeview(
            parent, columns=("Time", "ID", "Name", "Raw Data"), show="headings"
        )
        ...
        for c in ("Time", "ID", "Name", "Raw Data"):
            ...
            self.data_tree.column(c, anchor="center")
    ...
    def _setup_graph_view(self, parent):
        ...
        tk.Label(g_side, text="SELECT SIGNALS", bg="#dfe4ea", font=("Segoe UI", 8, "bold")).pack(
            fill=tk.X
        )
        ...
        scr_g.pack(side=tk.RIGHT, fill=tk.Y)
        self.g_canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
    ...
    def update_graph(self):
        ...
        for full_name, var in self.graph_check_vars.items():
            if var.get():
                ...
                if times:
                    self.ax.plot(
                        times, values, label=sig_name, marker="o", markersize=3, linewidth=1
                    )
                    has_data = True
    ...
    def _populate_graph_signals(self):
        for w in self.g_scroll_f.winfo_children():
            w.destroy()
        ...
        for msg in sorted(self.db.messages, key=lambda x: x.name):
            msg_lbl = tk.Label(
                self.g_scroll_f, text=msg.name, bg="#ced6e0", font=("Segoe UI", 8, "italic")
            )
    ...
    def apply_filter(self):
        ...
        for idx, item in enumerate(self.raw_log_data):
            ...
            if item["name"] not in self.checked_frames:
                continue
            ...
            self.data_tree.insert(
                "", "end", values=(item["ts"], item["id"], item["name"], item["hex"])
            )
            v_count += 1
    ...
    def on_select_msg(self, _):
        if not (sel := self.data_tree.selection()):
            return
    ...
    def log_message(self, message, level="INFO"):
        ...
        self.log_text.insert(
            tk.END, f"[{datetime.now().strftime('%H:%M:%S')}] [{level}] {message}\n"
        )
