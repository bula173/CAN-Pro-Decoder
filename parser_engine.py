...
class CANParser:
    ...
    @staticmethod
    def process_asc(path: str, db: cantools.database.Database, log_func: Optional[Callable] = None) -> List[Dict[str, Any]]:
        processed_data = []
        line_num = 0
        skipped_count = 0
        ...
        try:
            with open(path, 'r', encoding='utf-8', errors='ignore') as f:
                for line in f:
                    ...
                    if len(p) < 5 or any(x in p for x in ['Status:', 'Statistic:', 'internal', 'Begin', 'End']):
                        continue

                    try:
                        # 2. Find Direction anchor (Rx/Tx)
                        if 'Rx' in p: dir_idx = p.index('Rx')
                        elif 'Tx' in p: dir_idx = p.index('Tx')
                        else: continue

                        # 3. Extract ID (ID is immediately before Rx/Tx)
                        raw_id_str = p[dir_idx - 1].lower()
                        clean_id_str = raw_id_str.replace('x', '').replace('h', '').replace('0x', '')
                        ...
                        if len(payload_parts) < dlc:
                            _log(f"Line {line_num}: ID {raw_id_str} expected {dlc} bytes, but found {len(payload_parts)}.", "WARNING")
                            skipped_count += 1
                            continue
